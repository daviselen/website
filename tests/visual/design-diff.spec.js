...
const SECTIONS = [
  // humanai baseline carried over from the pre-redesign branch — stale vs the
  // current HumanAI.jsx; informational until a current @1x export is committed.
  { name: "humanai", selector: "#human-ai", contentBox: false, enforce: false },
  { name: "footer", selector: "#footer", contentBox: false, enforce: true },
  { name: "masthead", selector: "#masthead", contentBox: true, enforce: false },
  // proof baseline predates the current design (71.59% today, was 1.42% on
  // the branch this harness came from) — informational until re-exported.
  { name: "proof", selector: "#proof", contentBox: true, enforce: false },
  // portfolio-grid renders full-bleed at 1856: the redesign moved px-8 off
  // the <section> onto an inner div, so contentBox finds no padding to
  // strip and the 1792 export no longer lines up. Needs a 1856 re-export.
...
for (const { name, selector, contentBox, enforce } of SECTIONS) {
  test(`${name} distance from design`, async ({ page }, testInfo) => {
    ...
    if (dw > MAX_WIDTH_SLACK) {
      ...
      // Hard failure only where the section is gated. For informational
      // sections a width gap means the committed export predates a layout
      // change, which is the same "awaiting reconciliation" state the
      // enforce:false branch below reports — so log it rather than throwing
      // and masking every other section's number behind one error.
      if (enforce) {
        throw new Error(msg);
      }
      console.log(`${msg} INFORMATIONAL (enforce:false) — skipping diff until a matching export is committed.`);
      return;
    }
